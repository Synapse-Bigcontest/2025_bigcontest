...
def _load_festival_data():
    """
    축제 원본 CSV (festival_df.csv)를 로드하고 캐시합니다.
    """
    try:
        # 1번 제안: config 파일 사용
...
def get_festival_profile_by_name(festival_name: str) -> str:
    """
    축제 이름을 입력받아, 해당 축제의 상세 프로필 정보(JSON)를 반환합니다.
    축제 정보를 단순 조회할 때 사용됩니다. (예: "보령머드축제 정보 알려줘")
    """
...
def analyze_merchant_profile(store_profile: str) -> str:
    """
    가맹점 프로필(JSON 문자열)을 입력받아, 
    LLM을 사용하여 가게의 [강점/약점/기회요인]을 분석하고 요약 리포트를 반환합니다.
    (예: "내가게 분석해줘", "내가게 강점이 뭐야?")
    """
    ...
    try:
        ...
        [분석 가이드라인]
        1.  **강점 (Strengths)**: '동일 상권/업종 대비' 높은 수치(매출, 방문객, 객단가 등)나 '재방문율' 등을 찾아 강조하세요.
        2.  **약점 (Weaknesses)**: '동일 상권/업종 대비' 낮은 수치나 '신규 고객 비율' 등을 찾아 개선이 필요한 점을 언급하세요.
        3.  **기회 (Opportunities)**: 가게의 '주요 고객층'이나 '상권' 특성을 바탕으로, 어떤 마케팅(예: 특정 연령대 타겟, 신규 고객 유치)이 효과적일지 제안하세요.
...
def analyze_festival_profile(festival_name: str) -> str:
    ...
    try:
        # 1. Tool 1 (get_festival_profile_by_name) 호출
        profile_json = get_festival_profile_by_name.invoke({"festival_name": festival_name})
        ...
        [분석 가이드라인]
        1.  **핵심 특징**: 축제의 주제(키워드 기반)와 인기도(축제_인기도, 인기도_점수)를 1~2줄로 요약합니다.
        2.  **주요 방문객**: '주요_고객층'과 '주요_방문자' 컬럼을 직접 인용하여 설명합니다. 
            (예: [주요_고객층]이 주로 방문하며, [주요_방문자] 비율이 높습니다.)
